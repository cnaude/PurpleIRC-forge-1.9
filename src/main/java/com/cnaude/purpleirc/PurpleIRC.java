...
public class PurpleIRC {
    ...
    private void loadConfig() throws IOException {
        ...
        ircNickPrefixOwner = colorConverter.translateAlternateColorCodes('&', getConfig().getOption("nick-prefixes.owner", "@"));
    ...
    public String getMCPlayers(PurpleBot ircBot, String channelName) {
        ...
        for (EntityPlayerMP ep : fmlInstance.getServer().getPlayerList().getPlayerList()) {
    ...
    public void updateDisplayNameCache(EntityPlayerMP player) {
        logDebug("Caching displayName for " + player.getDisplayNameString() + " = " + player.getDisplayName());
        displayNameCache.put(player.getName(), player.getDisplayNameString());
    }
    ...
    public void broadcastToGame(final String message, final String permission) {
        for (EntityPlayerMP ep : fmlInstance.getServer().getPlayerList().getPlayerList()) {
            ep.addChatComponentMessage(new TextComponentTranslation(message));
        }
    }
