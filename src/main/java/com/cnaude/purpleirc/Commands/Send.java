    ...
    public void dispatch(CommandSender sender, String[] args) {
        if (args.length >= 2) {
            ...
            for (PurpleBot ircBot : myBots) {
                ...
                EntityPlayer ep = sender.getPlayer();

                if (channelName == null) {
                    for (String c : ircBot.botChannels) {
                        if (ep != null) {
                            ircBot.gameChat((EntityPlayerMP) sender.getPlayer(), c, msg.substring(1));
                        } else {
                    ...
                    if (ep != null) {
                        ircBot.gameChat((EntityPlayerMP) sender.getPlayer(), channelName, msg.substring(1));
                    } else {
